...
public class StudentLeave extends JFrame implements ActionListener {
    ...
    StudentLeave(){
        ...
        JLabel head = new JLabel("Search by Roll Number");
        head.setBounds(50, 80, 200, 25);
        head.setFont(new Font("serif",Font.PLAIN,20));
        add(head);

        rollNoField = new JTextField();
        rollNoField.setBounds(250, 80, 150, 25);
        add(rollNoField);
        ...
        ftime = new JComboBox(gfhfh);
        ftime.setBounds(100, 180, 150, 25);
        add(ftime);
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource()==submit){
            ...
            String date=((JTextField)dobc.getDateEditor().getUiComponent()).getText();
            String duration=(String)ftime.getSelectedItem();
            
            String query="insert into leavestudent values('"+rollno+"','"+date+"','"+duration+"')";
